...
char *
filename(char *buf, off_t *filesize)
{
...
void
write_header_handler(liby_client *client, char *buf, off_t length, int ec)
{
    if(!ec) {
    ...
    if(ec) {
        ...
        if(path) {
            ...
            if(data[0] >= 0) {
                ...
                liby_async_write_some(client, buf, length, write_header_handler);
                return;
            }
        }
    }
...
void
httpd_aceptor(liby_client *client)
{
